...
Once the data is downloaded to our computer we need to upload it into SAS.
```
...
KEEP Luxuriant AgeLuxuriant; \* Filtering out all the columns that aren't relevant 
...
Visually:
```
PROC SGPLOT DATA = PROJECT2.Luxurianttest;
	VBOX Length / Category = Group EXTREME;
	TITLE "Luxuriant vs Placebo";
	XAXIS LABEL = "Group";
	YAXIS LABEL = "Hair Growth (mm)";
RUN;
```
```{r luxuriant_v_test}
knitr::include_graphics("/Users/matthewcoudert/Maths/2020:21/MT5763/Project 2/Luxuriant vs Placebo.png")
```
...
Now comparing this observed difference to the difference of means in the randomly generated groups:
```
data ResultHolderabbrev;
set ResultHolder;
...
```
PROC SGPLOT DATA = PROJECT2.LuxuriantBaldBeGoneTest;
	VBOX Length / Category = Group EXTREME;
...
RUN;
```
```{r luxuriant_v_BaldBeGone}
knitr::include_graphics("/Users/matthewcoudert/Maths/2020:21/MT5763/Project 2/Luxuriant vs BaldBeGone.png")
```
...
```
PROC SGPLOT DATA = PROJECT2.LuxuriantSkinheadnomoretest;
	VBOX Length / Category = Group EXTREME;
...
```
```{r luxuriant_v_test}
knitr::include_graphics("/Users/matthewcoudert/Maths/2020:21/MT5763/Project 2/Luxuriant vs Skinheadnomore.png")
```
...
RUN;
```
This results in the fitted linear model:
...
It is clear from $\mathbb{P}(>t)=0.9655$ that age does not have a statistically significant impact on the effect of using hair growth treatment when viewing all the treatments in agregate. 

Helpfully, `PROC GLM` also generates this plot so we can visualize the effect age has on the data. Looking at the plot, the data appears to be pretty much randomly distributed along the age line which fits with our models output of no effect of age on hair growth treatment. As there is less data for both younger and older patients, the model is less confident in the fit line, so has a wider tail. 
```{r ageFitPlot}
knitr::include_graphics("/Users/matthewcoudert/Maths/2020:21/MT5763/Project 2/AgeFitPlot.png")
```

## Conclusion
LUXURIANT is an exciting new hairloss drug coming onto the market. The study undertaken provides strong evidence that Luxuriant has a significant positive effect on hair growth as compared to a placebo treatment. As compared to existing drugs on the market, BaldBeGone and SkinHeadNoMore Luxuriant is a weaker treatment, having a statistically significant weaker positive affect on hair growth. Finally the study has shown no evidence of an effect of age on the effectiveness of the hair growth treatments. That means patients at all stages of their lives can feel the benefits of Luxuriant as well as the other hair growth drugs. 
